...
const { sleep, delay, selectProp } = require('./util')
...
class MultipartUpload extends TaskZx {
  ...
  async start() {
    await super.start()
    let opt = Object.assign({}, this.getConf('refresh_token access_token,drive_id,userInfo,parent_file_id'), selectProp(this.opt, 'refresh_token drive_id parent_file_id'))
