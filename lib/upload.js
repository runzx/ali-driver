...
class MultipartUpload extends TaskZx {
  ...
  async download(fileName) {
    await this.initAliyunDriver()
    const files = await this.ali.aliPath(fileName)
    const dir = this.opt.out || ''
    for (const file of files) {
      const { content_hash, content_hash_name, download_url, size,
        url, name, type, file_id, parent_file_id, } = file
      const f = await stat(path.resolve(dir, path.basename(fileName))).catch(err => null)
      // await open(path.resolve(dir, path.basename(fileName)))
      const tempSize = f && f.size ? f.size : 0
      if (tempSize < size)
        let res = await downloadToFile(download_url, {
          fileName: path.basename(fileName), dir, flags: 'a', // 追加，续存； hash不用
          headers: {
            Referer: 'https://www.aliyundrive.com/',
            'content-type': '', 'Range': `bytes=${tempSize}-`
          }
        })
      const fileHash = await this.ali.getFileHash(path.resolve(dir, path.basename(fileName)), content_hash_name)
      if (content_hash === fileHash)
        console.log('%s download ok, size: %sMB', fileName, (size / 1048576).toFixed(3))
      else { }
    }
    // this.setConf('download', file)
  }
}
