 ...
 * @Last Modified time: 2021-10-26 23:20:18
 * 翟享 20180206
 * 
 * rp(url, opt)
...
const httpRp = async (url, { method = 'GET', timeout = TIMEOUT, headers, ...options } = {}, cb) => {
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => reject(`time out: ${timeout}S,  ${method}  ${url} \n ${JSON.stringify({ ...options, data: '' })}`), timeout * 1000)
 ...
 */
exports.downloadToFile = async (url, { dir, fileName, hash, flags = 'w', ...opt }, cb) => {
  ...
  const res = await exports.downloadToStream(url, opt, (chunk, headers) => {
    if (!chunk && !opt.stream) {
      if (dir && headers['content-disposition']) {
        ...
        tmp && (fileName = path.resolve(dir, decodeURIComponent(tmp[1])))  // 中文
      } else if (dir) {
        fileName = path.resolve(dir, fileName)
      }
      opt.stream = fs.createWriteStream(fileName, { flags })
    } else {
