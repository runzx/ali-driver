...
class AliyunDriver {
   ...
   * @param {object} param  limit 50 next_marker
   * @returns {[]} items
   * url expires 900s max 3600*4
   */
  async getFileList(parent_file_id = 'root', { url_expire_sec = 3600 * 4, next_marker, ...data } = {}) {
    ...
    let res = await this.post(url, { ...data, url_expire_sec, parent_file_id, next_marker },)
    if (res.data.next_marker && res.data.next_marker !== next_marker) {
      let next = await this.getFileList(parent_file_id, {
        ...data, url_expire_sec, parent_file_id,
        next_marker: res.data.next_marker
      })
      return res.data.items.concat(next)
    }
  ...
  async aliPath(dirPath, parent, deep = 10) {
    if (['/', '\\', '.', 'root', null, ''].includes(dirPath) && !parent) {
      return this.getFileList('root')
    }
    let dir = await this.getFileList(parent ? parent : 'root')
    let p = dirPath.split('/')
    if (['.', ''].includes(p[0])) p = p.slice(1)
    let res = dir.find(i => i.name === p[0])
    if (res) {
      if (p.length === 1) {
        if (res.type === 'file') return [res]
        return this.getFileList(res.file_id)
      } else if (deep > 0) return this.aliPath(p.slice(1).join('/'), res.file_id, deep - 1)
    }
    return dirPath
  }
}
